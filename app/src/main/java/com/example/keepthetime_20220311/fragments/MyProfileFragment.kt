...
import com.example.keepthetime_20220311.ManagePlaceActivity
...
class MyProfileFragment :BaseFragment(){
    ...
    override fun setupEvents() {
        ...
        binding.imgProfile.setOnClickListener {
            ...
            // 이미지 선택 화면으로 이동 - 안드로이드 제공 화면활용 :Intent(4)번 활용법 참고


            // 다른 화면에서 결과 받아오기 - Intent(3)번 활용법 참고  : startActivityForResult
            val myIntent =Intent()
        ...
        binding.btnManagePlace.setOnClickListener{

            val myIntent = Intent(mContext, ManagePlaceActivity::class.java)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == REQ_CODE_GALLERY){
            if(requestCode == Activity.RESULT_OK){
                ...
                // data? 선택된 사진에 대한 정보를 가지고있다.
                val selectedImageUri = data?.data!! // 선택한 사진에 찾아갈경로(Uri) 받아내기

                // Uri -> 실제 첨부 가능한 파일 형태로 변환 (File객체를, Path를 통해 만든다)-> Retrofit에 첨부할수 있게 됨

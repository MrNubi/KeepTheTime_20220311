...
class EditAppointmentActivity : BaseActivity() {
    ...
    var path : PathOverlay? = null  // 출발지~ 도착지까지 보여줄 경로선,  처음에는 보이지않는 상태
    ...
    override fun SetupEvents() {
        ...
        binding.startPlaceSpinner.onItemSelectedListener = object :AdapterView.OnItemSelectedListener{
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                ...
                mSelectedStartPlace = mStartPlaceList[p2]
        ...
        // 스크롤 보조용 텍스트뷰에 손이 닿으면 => 스크롤뷰의 이벤트 일시 정지 (지도만 움직이게)
        ...
        binding.btnSave.setOnClickListener {
            ...
            val now =  Calendar.getInstance()  //  저장버튼을 누른 현재시간
            ...
            if(mSelectedLatLng == null){
                Toast.makeText(mContext, "약속장소를 선택하지 않았습니다", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
...
//                Log.d("클릭된 위/경도", "위도:${latLng.latitude},  경도:${latLng.longitude}")
            ...
            // coord ~ 선택한 lating까지 대중교퉁 경로를 그려보자 (PathOverlay 기능활용)  + ODSay 라이브러리 활용
        ...
        myODsayService.requestSearchPubTransPath(
            ...
            object :OnResultCallbackListener{
                override fun onSuccess(p0: ODsayData?, p1: API?) {
                    ...
                    val pathArr = resultObj.getJSONArray("path")  // 여러추천 경로 중 첫번째거만 사용해보자
                    ...
                    for(i in 0 until subPathArr.length()){
                        ...
                        if(!subPathObj.isNull("passStopList")){
                            ...
                            // 실제 정거장 목록 파싱 => 각 정거장의 위도/경도 추출가능 => ArrayList에 담아서, 경로선의 좌표로 활용
...
//                            Toast.makeText(mContext, "시간: ${totalTime}분,  비용: ${payment}원", Toast.LENGTH_SHORT).show()
                    ...
                    //연습문제- 카메라를 latLng(클릭한위치)가 가운데로 오도록 세팅
                    // 공식문서 활용 연습문제

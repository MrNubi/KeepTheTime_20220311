...
//            txtEmail.text = data.email
            ...
            when(data.provider){
                ...
                "facebook"->{
                    imgSocialLoginLogo.visibility =View.VISIBLE
                    imgSocialLoginLogo.setImageResource(R.drawable.img_facebook)
                    txtEmail.text ="페이스북 로그인"
                }
                "naver"->{
                    imgSocialLoginLogo.visibility =View.VISIBLE
                    imgSocialLoginLogo.setImageResource(R.drawable.img_naver)
            ...
            // 친구 추가 버튼이 눌리면 할일 => 친구추가요청 API호출
            // 어댑터에서 => API를 호출? =>레트로픽 객체 직접 생성해서 호출
...
//            apiList.
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        ...
        // 재사용성을 알아서 보존해줌
        val row = LayoutInflater.from(mContext).inflate(R.layout.serached_user_list_item, parent, false)
